...
export const Route = createFileRoute("/api/integrations/steadfast/create-consignment")({
  server: {
    handlers: {
      ...
      POST: async ({ request }) => {
        ...
        if (!body.payload) return jsonBad("Consignment payload missing");
        return jsonOk(await forward(`${base}/create_order`, {
          method: "POST",
