...
<style>
  ...
  <h2>Salary Slip — ${slip.period_month}</h2>
  <div class="row"><div><b>Employee:</b> ${slip.employees?.name || ""} ${slip.employees?.code ? "(" + slip.employees.code + ")" : ""}</div><div class="muted">Dept: ${slip.employees?.department || "—"}</div></div>
  <div class="row"><div>Days Present</div><div>${slip.days_present} / ${slip.days_total}</div></div>
  <div class="row"><div>Status</div><div>${slip.status.toUpperCase()} ${slip.paid_on ? "· " + slip.paid_on : ""}</div></div>
  <table>
    <tr><td>Gross</td><td class="right">৳ ${fmt(slip.gross)}</td></tr>
    <tr><td>+ Bonus</td><td class="right">৳ ${fmt(slip.bonus)}</td></tr>
    <tr><td>− Deductions</td><td class="right">৳ ${fmt(slip.deductions)}</td></tr>
    <tr><td>− Advance Recovery</td><td class="right">৳ ${fmt(slip.advance)}</td></tr>
    <tr class="total"><td>Net Payable</td><td class="right">৳ ${fmt(slip.net)}</td></tr>
  </table>
  <div class="row" style="margin-top:16px"><div class="muted">Payment Method</div><div>${(meta.method || "—").toString().toUpperCase()}</div></div>
